...
Để đáp ứng yêu cầu của môn học, nhóm đã áp dụng các kỹ thuật C++ nâng cao sau:
...
Nhóm thực hiện gồm 4 sinh viên, tuân thủ đúng cam kết đóng góp thực sự vào sản phẩm.
Thành viên                    Nhiệm vụ chính
Quan (Nhóm trưởng)            Xây dựng giao diện, xử lý File I/O.
Quý                           Xây dựng logic các cổng, xử lý tín hiệu, xây dựng dây nối và truyền tín hiệu
Thịnh                         Xử lý đồ họa (QGraphicsItem), tương tác dây nối và logic chân cắm.Thành viên DSoạn thảo báo cáo kỹ thuật (UML, Flowchart), Kiểm thử (Testing).
